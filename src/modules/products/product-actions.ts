...
export async function createProductAction(
  ...
  if (data.categoryId) {
    const categoryExists = await prisma.category.count({
      where: { id: data.categoryId, companyId: company.id, status: "ACTIVE" },
    });
    if (!categoryExists) {
      return { status: "error", message: "La categoría seleccionada ya no está disponible." };
    }
  }

  if (data.brandId) {
    const brandExists = await prisma.brand.count({
      where: { id: data.brandId, companyId: company.id, status: "ACTIVE" },
    });
    if (!brandExists) {
      return { status: "error", message: "La marca seleccionada ya no está disponible." };
    }
  }
  ...
  try {
    ...
    return {
      status: "error",
      message: "No se pudo guardar el producto. Revisa la conexión con PostgreSQL e inténtalo nuevamente.",
    };
  }

...
export function PosFormV2({
  ...
  const paymentComplete = pendingAmount <= 0.01 && !invalidOverpayment;
      ...
      <div className="pos-layout">
        ...
        <aside className="pos-checkout">
            ...
            <div className="payment-lines">
              {payments.map((payment, index) => (
                <div className="payment-line" key={payment.id}>
                  ...
                  <label className="payment-field">
                    <span>{payment.method === "CASH" ? "Efectivo recibido" : "Importe"}</span>
                    <input
                      type="number"
                      min="0"
                      step="0.01"
                  ...
                  <label className="payment-field reference">
                    ...
                    <input
                      ...
                      placeholder={payment.method === "CASH" ? "Opcional" : "N.º de operación (opcional)"}
                    />
                  </label>
          ...
          <section className="panel pos-total-card">
            ...
            <div className={`payment-balance ${invalidOverpayment ? "pending" : change > 0.01 ? "change" : paymentComplete ? "ok" : "pending"}`}>
              <span>
                {invalidOverpayment ? "Revisa el exceso de pago" : paymentComplete ? "Cobro cubierto" : "Cobro pendiente"}
                <small>Efectivo recibido: {money(cashReceived)}</small>
              </span>
              <strong>
                {invalidOverpayment
            ...
            <p className="tax-note">
              Los precios de MOBIX son precios finales. En operaciones gravadas el sistema desglosa el IGV del 18% incluido en el total. Si el cliente entrega más efectivo, MOBIX calcula el vuelto automáticamente.
            </p>
            <button
              ...
              onClick={submitSale}
            >
              {isPending ? "Procesando venta..." : change > 0.01 ? `Confirmar venta · Vuelto ${money(change)}` : "Confirmar venta"}
            </button>
            <p className="form-footnote">
              MOBIX registra en caja el efectivo neto de la venta; el importe entregado por el cliente y el vuelto se calculan en el POS. Stock, IMEI, Kardex, pagos y auditoría se procesan en una sola transacción.
            </p>
          </section>
        </aside>
      </div>

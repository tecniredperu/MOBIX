...
export async function getReports(filters: { from?: string; to?: string }) {
  ...
  for (const sale of sales) {
    ...
    day.sales += Number(sale.total);
    day.cost += saleCost;
    day.profit += Number(sale.total) - saleCost;
    day.count += 1;
    daily.set(key, day);

    const seller = sellers.get(sale.sellerId) ?? { seller: sale.seller.name, sales: 0, profit: 0, count: 0 };
    seller.sales += Number(sale.total);
    seller.profit += Number(sale.total) - saleCost;
    seller.count += 1;
    sellers.set(sale.sellerId, seller);

    const branch = branches.get(sale.branchId) ?? { branch: sale.branch.name, sales: 0, profit: 0, count: 0 };
    branch.sales += Number(sale.total);
    branch.profit += Number(sale.total) - saleCost;
    branch.count += 1;
    branches.set(sale.branchId, branch);

    for (const payment of sale.payments) {
      payments.set(payment.paymentMethod, (payments.get(payment.paymentMethod) ?? 0) + Number(payment.amount));
    }
    ...
    for (const item of sale.items) {
      ...
      row.cost += itemCost;
      row.profit += Number(item.total) - itemCost;
  ...
  const inventoryValue = accessoryBalances.reduce(
  ...
  return {
    ...
    summary: {
      ...
      costTotal,
      grossProfit,
      margin: salesTotal ? (grossProfit / salesTotal) * 100 : 0,

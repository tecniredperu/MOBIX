...
export function ReportsView({data}:{data:any}){
  ...
  return <div className="page-stack report-page">
    ...
    <section className="report-kpis">
      ...
      {data.canSeeCosts&&<article><TrendingUp/><span>Utilidad bruta</span><strong>{money(data.summary.grossProfit)}</strong><small>Margen {pct(data.summary.margin)}</small></article>}
      {data.canSeeCosts&&<article><WalletCards/><span>Costo vendido</span><strong>{money(data.summary.costTotal)}</strong><small>Compras del periodo {money(data.summary.purchasesTotal)}</small></article>}
    ...
    <section className="report-grid-two">
      <article className="panel report-chart"><div className="panel-heading"><div><h2>Ventas por día</h2><p>{data.canSeeCosts?"Ingresos y utilidad del periodo":"Ingresos del periodo"}</p></div></div><div className="report-bars">{data.daily.length?data.daily.map((d:any)=><div className="report-bar-row" key={d.date}><span>{new Date(`${d.date}T12:00:00`).toLocaleDateString("es-PE",{day:"2-digit",month:"short"})}</span><div><i style={{width:`${Math.max(2,(d.sales/maxDaily)*100)}%`}}/></div><strong>{money(d.sales)}</strong>{data.canSeeCosts&&<small>Util. {money(d.profit)}</small>}</div>):<div className="empty-table-state">Sin ventas en el periodo.</div>}</div></article>

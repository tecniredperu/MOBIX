...
async function getMembership(companyId: string) {
  ...
  if (!membership) throw new Error("No existe un usuario activo para registrar postventa.");
  return membership;
}
...
export async function createServiceOrderAction(input: {
  ...
  const serviceNumber = await prisma.$transaction(async (tx) => {
    await tx.$queryRaw<Array<{ locked: number }>>`
      ...
      FROM pg_advisory_xact_lock(hashtext(${`${company.id}:service-order`}))
    `;
    const sequence = await tx.$queryRaw<SequenceRow[]>`
      ...
      WHERE "companyId" = ${company.id}
    `;
    const next = Number(sequence[0]?.next ?? 1);
    const number = `ST001-${String(next).padStart(6, "0")}`;
...
export async function updateServiceOrderAction(input: {
  ...
  if (input.technicianId) {
    const technician = await prisma.companyUser.findFirst({
      where: { companyId: company.id, userId: input.technicianId, status: "ACTIVE", user: { status: "ACTIVE" } },
      select: { userId: true },
    });
  ...
  const rows = await prisma.$queryRaw<ExistingOrderRow[]>`
    ...
    FROM "service_orders"
    WHERE "id" = ${input.orderId} AND "companyId" = ${company.id}
    LIMIT 1
  `;
  const order = rows[0];
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.$executeRaw`
      ...
      SET
        ...
        "updatedAt" = NOW()
      WHERE "id" = ${order.id} AND "companyId" = ${company.id}
    `;

    if (order.status !== input.status || input.note?.trim()) {
      await tx.$executeRaw`
        INSERT INTO "service_order_events" ("id", "serviceOrderId", "status", "note", "createdById", "createdAt")
        VALUES (${randomUUID()}, ${order.id}, ${input.status}, ${input.note?.trim() || null}, ${membership.userId}, NOW())
      `;
    }
    ...
    if (order.productUnitId && ["DELIVERED", "CANCELLED"].includes(input.status)) {
      const unit = await tx.productUnit.findFirst({
        ...
        select: { id: true, warehouseId: true, productId: true, variantId: true, purchaseCost: true, status: true },
      });

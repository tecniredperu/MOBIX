...
export function ServiceForm({ customers, soldUnits }: { customers: ServiceCustomerOption[]; soldUnits: SoldUnitOption[] }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        const response = await fetch(`/api/service/sold-units?q=${encodeURIComponent(unitQuery.trim())}`, { signal: controller.signal, cache: "no-store" });
        if (!response.ok) throw new Error("No se pudieron buscar los equipos vendidos.");
        const payload = await response.json() as { items: SoldUnitOption[] };
        setAvailableSoldUnits((current) => {
          const selected = current.find((item) => item.id === unitId);
          return selected && !payload.items.some((item) => item.id === selected.id) ? [selected, ...payload.items] : payload.items;
        });
      } catch (cause) {
        if (!controller.signal.aborted) setError(cause instanceof Error ? cause.message : "No se pudieron buscar los equipos vendidos.");
      } finally {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        const response = await fetch(`/api/service/customers?q=${encodeURIComponent(customerQuery.trim())}`, { signal: controller.signal, cache: "no-store" });
        if (!response.ok) throw new Error("No se pudieron buscar los clientes.");
        const payload = await response.json() as { items: ServiceCustomerOption[] };
        setAvailableCustomers((current) => {
          const selected = current.find((item) => item.id === customerId);
          return selected && !payload.items.some((item) => item.id === selected.id) ? [selected, ...payload.items] : payload.items;
        });
      } catch (cause) {
        if (!controller.signal.aborted) setError(cause instanceof Error ? cause.message : "No se pudieron buscar los clientes.");
      } finally {
    ...
    startTransition(async () => {
      try {
        const result = await createServiceOrderAction({
          ...
          reportedIssue, accessories, physicalCondition,
          estimatedCost: Number(estimatedCost || 0), expectedAt,
        });
  ...
  return (
    <div className="page-stack service-form-page">
      <section className="page-heading"><div><Link href="/servicio-tecnico" className="back-link"><ArrowLeft size={15} /> Volver a postventa</Link><span className="eyebrow">POSTVENTA</span><h1>Nueva atención</h1><p>Registra el equipo, condición de ingreso, falla reportada y fecha estimada de entrega.</p></div></section>

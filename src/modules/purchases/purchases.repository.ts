...
export async function getPurchases(filters: { q?: string; status?: string } = {}) {
  ...
  const [purchases, totalCount, totals] = await Promise.all([
    prisma.purchase.findMany({
      ...
      select: {
        ...
        status: true,
        notes: true,
        supplier: { select: { businessName: true, documentNumber: true } },
        warehouse: {
          select: {
            name: true,
  ...
  return {
    items: purchases.map((purchase) => ({
      ...
      supplier: purchase.supplier?.businessName ?? "Proveedor no disponible",
      ...
      warehouse: purchase.warehouse?.name ?? "Almacén no disponible",
      branch: purchase.warehouse?.branch?.name ?? "Sucursal no disponible",

    ...
    <div className={`mobile-nav-layer${open ? " open" : ""}`} aria-hidden={!open}>
      ...
      <aside className="mobile-nav-drawer">
        ...
        <nav className="mobile-nav-list">
          ...
          {(showAdmin || showAudit || allowed("settings.manage")) && <section><p>Sistema</p>{showAdmin && <Link href={adminHref} className={active(pathname,adminHref)?"active":""}><ShieldCheck size={18}/><span>Administración</span></Link>}{showAudit && <Link href="/administracion/auditoria" className={active(pathname,"/administracion/auditoria")?"active":""}><ScrollText size={18}/><span>Auditoría</span></Link>}{allowed("settings.manage") && <Link href="/configuracion" className={active(pathname,"/configuracion")?"active":""}><Settings size={18}/><span>Configuración</span></Link>}</section>}
        </nav>
